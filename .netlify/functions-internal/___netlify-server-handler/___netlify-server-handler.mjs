...
} from '/var/task/lumina-factory/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/lumina-factory/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/lumina-factory')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/lumina-factory',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/lumina-factory/.netlify/dist/run/handlers/server.js')
